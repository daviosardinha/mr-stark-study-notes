---
title: Enterprise Admins Trust - SID-History Abuse with Trust Ticket
layout: docs
---

### What is SID-History Abuse?

SID-History Abuse is a technique that allows an attacker to escalate privileges and bypass access controls in Windows systems. It involves injecting higher-privileged Security Identifiers (SIDs) into the SID History attribute of a user account, effectively granting the user the permissions associated with those higher-privileged groups.
...
### SID-History Abuse using Trusted Tickets

The search results indicate that SID-History Abuse can be combined with the abuse of trusted tickets to further escalate privileges across domain boundaries:

1. Compromising a Child Domain: An attacker first compromises a user account in a child domain within the same forest.

2. Adding Parent Domain SIDs: The attacker adds the SID of a higher-privileged group from the parent domain (e.g., Enterprise Admins) to the SID History of the compromised user account.

3. Forging a Trusted Ticket: The attacker then forges a Kerberos ticket that can be used to access resources in the parent domain, leveraging the added SIDs in the SID History.

This combination of SID-History Abuse and trusted ticket forgery allows the attacker to effectively compromise the entire forest, as long as they can gain a foothold in a single child domain.

# Enumeration

Ok so assuming that we already have Access as Domain Admin and we would like to privilege escalate to Enterprise Admin.
...
The output from `Get-DomainTrust` provides detailed information about the forest trust between `dollarcorp.moneycorp.local` and `moneycorp.local`. 
Here's a breakdown of the fields:

### Source and Target Names

1. SourceName: The name of the source domain, which is dollarcorp.moneycorp.local.

2. TargetName: The name of the target domain, which is moneycorp.local.

### Trust Type and Attributes

1. TrustType: The type of trust, which is WINDOWS_ACTIVE_DIRECTORY, indicating that it is a Windows Active Directory trust.

2. TrustAttributes: The attributes of the trust, which are WITHIN_FOREST, indicating that the trust is within the same forest.

### Trust Direction

1. TrustDirection: The direction of the trust, which is Bidirectional, indicating that the trust is bidirectional, allowing both domains to access resources in each other.

Now that we have done the enumeration and we know exactly how is the Child/Parent domain trust configured, let’s carry on.
We wont be doing this attack directly from de Domain Controller. We will be executing everything remotely from our Target machine.
...
Above we can see that we were able to dump all keys/passwords for all associated trusts. Our focus so far is on child/Parent domain trust (I have market the important one as Red above).
...
`C:\AD\Tools>C:\AD\Tools\Loader.exe -path C:\Ad\Tools\Rubeus.exe -args %Pwn% /service:krbtgt/dollarcorp.moneycorp.local /rc4:11584b767b6316e5f48b35e029c32ade /sid:S-1-5-21-719815819-3726368948-3917688648 /sids:S-1-5-21-335606122-960912869-3279953914-519 /ldap /user:administrator /nowrap`

```PowerShell

```

The flags used while using Rubeus to forge a ticket with SID-History of Enterprise Admin are:
...
2. /rc4: This flag specifies the RC4 key used to encrypt the ticket(dollarcorp.moneycorp.local). In this case, it is 11584b767b6316e5f48b35e029c32ade.
...
4. /sids: This flag specifies additional SIDs that should be included in the ticket(moneycorp.local). In this case, it is S-1-5-21-335606122-960912869-3279953914-519.

5. /ldap: This flag specifies that the ticket should be forged using LDAP.

6. /user: This flag specifies the username of the user that the ticket is being forged for. In this case, it is administrator.

7. /nowrap: This flag specifies that the output should not be wrapped.

8. /ptt: This flag specifies that the ticket should be printed to the console.

This command forges a ticket for the `krbtgt/dollarcorp.moneycorp.local` service using the RC4 key `11584b767b6316e5f48b35e029c32ade`, the SID `S-1-5-21-719815819-3726368948-3917688648`, and the additional SID `S-1-5-21-335606122-960912869-3279953914-519`. The ticket is forged using LDAP and the username is `administrator`. The output is not wrapped and the ticket is printed to the console.

if our payload works fine, then we have a base64 ticket. We can then use the same base64 ticket to request TGS from Parent Domain (moneycorp.local) Domain Controller, inject the ticket into our Cached Ticket and access the MoneyCorp.local Domain Controller.
...
Using Rubeus to request TGS for MCORP.LOCAL DC.

`C:\AD\Tools>C:\AD\Tools\Loader.exe -path C:\Ad\Tools\Rubeus.exe -args %Pwn% /service:http/mcorp-dc.moneycorp.local /dc:mcorp-dc.moneycorp.local /ptt /ticket:doIF7jCCBeqgAwIBB..BASE64_SNIPPET>`

Now if we check our Cached Ticket.

`klist`

Above we can see that we do have the ticket injected into our current session. Let’s confirm it by tring to access **MCORP-DC** inside moneycorp.local domain using winrs.

`winrs -r:mcorp-dc.moneycorp.local cmd`

Above we were able to access the moneycorp DC, so we were able to abuse Domain Trust using Trusted Ticket.