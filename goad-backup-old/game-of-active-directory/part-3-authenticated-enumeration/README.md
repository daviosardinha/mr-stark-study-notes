...
The results of our NetExec SMB enumeration provide a perfect illustration of why even a simple directory dump is so critical to our methodology. While deeper LDAP queries can provide us with complex bitmasks like the `userAccountControl`, the standard SMB output seen here focuses our attention on the most immediate high-value fields.
...
We have established the foundational state where our authenticated identity, no matter how limited its role, allows us to systematically mine the Active Directory database. When we move to performing LDAP searches as an authenticated user, we shift away from the restrictive queries used in our earlier anonymous reconnaissance. While anonymous binds were often blocked or limited by the modern defensive posture we encountered earlier, our current status as a "Domain User" provides us with a valid security principal to authenticate our queries through the LDAP Bind operation. We utilize the classic **ldapsearch** utility to achieve this, providing our confirmed credentials via the `-D` flag for the Distinguished Name and the `-w` flag for the password. This transition is significant because it unlocks the full breadth of the directory’s attributes which are strictly concealed from unauthenticated actors.
...
The most tactically rewarding part of our manual LDAP extraction is our focused hunt within the **Description** field. Since we established earlier that administrators in this forest frequently misuse the description attribute for support notes, we employ a filter such as `grep "description" -A3`. The use of the `-A` (After) flag in our search is a professional tradecraft choice. Active Directory entries are often multi-line, and descriptive notes frequently spill over the initial line. By capturing the three lines of context immediately following a description hit, we ensure we are not missing a password that might be separated from its label by a newline. This specific filter is exactly how we identify errors like the cleartext password we previously looted from Samwell Tarly.

`ldapsearch -H ldap://winterfell.north.sevenkingdoms.local -D 'brandon.stark@north.sevenkingdoms.local' -w 'iseedeadpeople' -b 'DC=north,DC=sevenkingdoms,DC=local' "*" | grep 'description:' -A3`
...
`ldapsearch -H ldap://kingslanding.sevenkingdoms.local -D 'brandon.stark@north.sevenkingdoms.local' -w 'iseedeadpeople' -b 'DC=sevenkingdoms,DC=local' "*" | grep 'userPrincipalName:'`
...
## Tombstones Users Enumeration `ldapsearch`

We are now shifting our attention to a fascinating area of Active Directory forensics that we treat as a "**digital archaeology**" phase of our engagement. We must understand that in the world of Active Directory, an object is rarely truly gone. When an administrator deletes a user, group, or computer account, the operating system performs a logical deletion rather than a physical removal from the `ntds.dit` database. The object is stripped of its non-essential attributes, marked as deleted, and moved to a hidden, protected container. As high-level operators, we recognize that this graveyard of "Tombstoned" objects represents a massive intelligence opportunity, providing us a look into the domain’s past that can directly facilitate our path to a full compromise.
...
We now shift to Winterfell (10.4.10.11) as a Domain Admin to establish our forensic misconfiguration and stage the bait object. We utilize a SYSTEM-elevated task to grant Brandon Stark the specific extended right required to perform the tombstone search.

We utilize the **`NT AUTHORITY\SYSTEM`** identity to perform our ACL modifications because the graveyard container is protected by an inherited system lock that typically blocks even local Domain Admins from modifying its security descriptors. By using a scheduled task to impersonate the operating system itself, we gain "Identity Level Zero" authority, allowing us to force our permission entries into the directory’s binary database without being restricted by the management console's default guardrails.
...
Once these commands are executed, the stage is perfectly set for our demonstration. When we return to our Kali attack box and initiate our `ldapsearch` using Brandon Stark’s credentials and the `1.2.840.113556.1.4.417` OID, the result will be transformative. Instead of an access denied error, we will retrieve the full profile of **reaper_svc**. We will see exactly when it was deleted, its original parent OU, and the crowning jewel, the cleartext "Legacy Pass" that we planted in the description.
...
As we can see from our enumeration above, we are able to see a bunch of Objects that hae been deleted on the last 180 days. We can also start filtering out the junk info and bring more accurate information when we are doing this time of enumeration.
...
By placing this file, we have created an inevitable point of compromise for any attacker who performs authenticated reconnaissance. From an architectural perspective, we have essentially "weaponized" the read access inherent to the Authenticated Users group. We didn't exploit a bug in the code; we revived an insecure feature that still resides in the OS kernel's memory for backward compatibility.

Our next operational move is to return to our Kali attack infrastructure. We will use the valid credentials we previously established for brandon.stark and utilize NetExec to perform the surgical sweep. 
NetExec is capable of login into the target and simply make this enumeration and automatically work on the whole process from finding the .xml file inside `SYSVOL` directory to decryption and bring us the ClearText password.
...
### **Python ingestor - from linux/BloodHound/BloodHound/BloodHound**
